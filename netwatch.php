...
<style>
    ...
    .status-intermittent { background: rgba(245, 158, 11, 0.15); color: #f59e0b; border: 1px solid rgba(245, 158, 11, 0.2); }
...
<div class="grid" style="grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 1.5rem; margin-bottom: 2rem;">
    <?php
    $count_up = 0; $count_down = 0; $count_intermittent = 0; $count_total = count($targets);
    foreach($targets as $t) {
        if($t['status'] == 'up') $count_up++;
        if($t['status'] == 'down') $count_down++;
        if($t['status'] == 'intermittent') $count_intermittent++;
    }
    ?>
    <div class="stat-card">
    ...
    <div class="stat-card">
        <span class="stat-label">Hosts Intermittent</span>
        <span class="stat-value" style="color: #f59e0b;"><?php echo $count_intermittent; ?></span>
        <span class="stat-meta">Unstable connection</span>
    </div>
</div>
            ...
            historyChart = new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        pointBackgroundColor: data.status.map(s => s === 'down' ? '#f85149' : (s === 'intermittent' ? '#f59e0b' : '#58a6ff'))
                    }]
                },
                options: {
                    responsive: true,
